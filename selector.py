...
class FileSelector:
    ...
    def apply_search_filter(self):
        ...
        try:
            ...
            # 전체 노드 목록 가져오기
            all_nodes = flatten_tree(self.root_node)
            ...
            if not matching_nodes:
                self.visible_nodes = self.original_nodes
                self.stdscr.addstr(0, self.width - 25, "검색 결과 없음", curses.color_pair(6))
                self.stdscr.refresh()
                curses.napms(1000)
                return
            ...
            self.visible_nodes = [
                ...
                if node in visible_nodes_set or (node.is_dir and node.children and any(child in visible_nodes_set for child in node.children.values()))
            ]
            
            # 인덱스 조정
            self.current_index = 0 if self.visible_nodes else 0
            
        except re.error:
            # 잘못된 정규식
            self.stdscr.addstr(0, self.width - 25, "잘못된 정규식", curses.color_pair(6))
            self.stdscr.refresh()
            curses.napms(1000)
            # 검색 실패 시 원래 목록 유지
            self.visible_nodes = self.original_nodes
